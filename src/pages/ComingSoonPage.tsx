...
const FloatingWord = ({ delay, x, color, word, fontSize }: {
  delay: number; x: number; color: string; word: string; fontSize: number;
}) => {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      void controls.start({
        ...
        x: [0, Math.sin(delay) * 60, Math.cos(delay) * -40, Math.sin(delay) * 30],
  ...
  return (
    <motion.span
      ...
      style={{
        ...
        left: `${x}%`,
        fontSize,
        fontFamily: word === 'ಪ್ರಜಾಕೀಯ' ? '"Noto Sans Kannada", sans-serif' : '"Playfair Display", serif',
        fontWeight: 700,
        color,
        opacity: 0,
        pointerEvents: 'none',
...
const GlowRing = ({ radius, delay, color }: { radius: number; delay: number; color: string }) => {
  const controls = useAnimation();
...
const ComingSoonPage = () => {
  const wordColors = [BRAND.red, BRAND.yellow, BRAND.yellow2, BRAND.saffron, BRAND.red2];
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Box component="span" sx={{ color: BRAND.yellow, fontSize: '0.8rem', opacity: 0.4 }}>|</Box>
        <MuiLink
          component={Link}
