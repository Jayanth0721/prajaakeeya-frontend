...
import {
    Box, Typography, Button, Divider, useTheme, useMediaQuery, Link,
} from '@mui/material';
import {
    InfoOutlined, HandshakeOutlined, VerifiedUserOutlined, PersonOutlined,
    ChatOutlined, HowToVoteOutlined, GroupOutlined, WarningOutlined,
    BlockOutlined, ArticleOutlined, FlagOutlined, MediationOutlined,
    SecurityOutlined, PublicOutlined, UpdateOutlined, MailOutlined,
    AssignmentOutlined,
        ...
        },
        {
            ...
            subsections: [
                ...
                },
                {
                    ...
                    paragraphs: [t(`${p}.s10.ss3.p1`), t(`${p}.s10.ss3.p2`), t(`${p}.s10.ss3.p3`)],
                },
                {
                    title: t(`${p}.s10.ss4.title`),
                    paragraphs: [t(`${p}.s10.ss4.p1`), t(`${p}.s10.ss4.p2`), t(`${p}.s10.ss4.p3`)],
                },
                {
...
function BulletList({ items, isDark }: { items: BulletItem[]; isDark: boolean }) {
    return (
        <Box component="ul" sx={{ m: 0, pl: 0, listStyle: 'none' }}>
            {items.map((item, i) => (
                <Box key={i} component="li" sx={{ mb: item.sub ? 0.5 : 0.25 }}>
                    ...
                    {item.sub && (
                        <Box component="ul" sx={{ m: 0, pl: 3.5, listStyle: 'none', mt: 0.5 }}>
                            {item.sub.map((s, j) => (
                                <Box key={j} component="li" sx={{ display: 'flex', alignItems: 'flex-start', gap: 1, mb: 0.25 }}>
                                    <Box
                                        component="span"
                                        sx={{
                                            mt: '8px', flexShrink: 0,
                                    ...
                                    <Typography
                                        sx={{
                                            fontSize: '0.87rem',
                                            lineHeight: 1.7,
                                            color: isDark ? 'rgba(255,255,255,0.65)' : 'rgba(17,24,39,0.7)',
                                        }}
                                    >
                                        {s}
                                    </Typography>
    ...
    return (
        <Box sx={{ minHeight: '100vh', background: pageBg, position: 'relative', pb: 8 }}>
                ...
                >
                    <Box sx={{
                        ...
                        <Box sx={{
                            ...
                            <Box component="img" src={prajakeeyaLogo} alt={t('communityGuidelines.header.logoAlt')}
                    ...
                    <Box>
                        ...
                        {!isMobile && (
                            <Typography sx={{
                                fontFamily: '"Tiro Kannada", serif', fontSize: '0.75rem',
                                color: dimText, letterSpacing: '2px',
                            }}>
            ...
            <Box
                ...
                sx={{
                    maxWidth: 860,
                    mt: isMobile ? 3 : 4, mb: 3,
                    mx: 'auto', px: isMobile ? 2 : 4,
                }}
            >
                <Box sx={{
                    background: cardBg,
                ...
                >
                    ...
                    <Box sx={{ px: isMobile ? 2.5 : 4, pb: 4, pt: 2 }}>
                        <Divider sx={{ borderColor: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(17,24,39,0.08)', mb: 2.5 }} />
                        <Typography sx={{
                            fontSize: '0.95rem',
                            color: bodyText,
                            lineHeight: 1.8,
                ...
                {sections.map((section) => (
                    ...
                    >
                        ...
                        <Box sx={{
                            ...
                            <Typography sx={{
                                flex: 1, fontSize: isMobile ? '0.95rem' : '1.1rem', fontWeight: 700,
                        ...
                        <Box sx={{ px: isMobile ? 2.5 : 4, py: 2.5 }}>
                            {section.paragraphs && section.paragraphs.map((para, i) => (
                                <Typography key={i} sx={{
                                    ...
                                    color: bodyText,
                                    mb: section.bullets && i === section.paragraphs!.length - 1 ? 1.5 : 1,
                                }}>
                                    {para}
                                </Typography>
                            ))}
                            {section.bullets && <BulletList items={section.bullets} isDark={isDark} />}
                            {section.subsections && section.subsections.map((subsec, i) => (
                                <Box key={i} sx={{ mt: subsec.title ? 2 : 1.5 }}>
                                    ...
                                    {subsec.paragraphs && subsec.paragraphs.map((para, j) => (
                                        <Typography key={j} sx={{
                                            fontSize: '0.92rem', lineHeight: 1.8,
                                            color: bodyText,
                                            mb: subsec.bullets && j === subsec.paragraphs!.length - 1 ? 1.5 : 1,
                                        }}>
                                            {para}
                                        </Typography>
                                    ))}
                                    {subsec.bullets && <BulletList items={subsec.bullets} isDark={isDark} />}
                                </Box>
                            ))}
                        </Box>
                    </Box>
            ...
            {/* ── Footer ── */}
            <Box sx={{
                mt: 6, pt: 4, pb: 3, px: isMobile ? 2 : 4,
                flexShrink: 0,
                textAlign: 'center',
                color: dimText,
                fontSize: '0.8rem',
                ...
                <Typography sx={{ fontSize: '0.8rem', color: dimText }}>
                    {t('communityGuidelines.footer.copyright', { year: new Date().getFullYear() })}
                </Typography>
                <Typography sx={{ color: dimText, fontSize: '0.78rem', mt: 0.5 }}>
                    {i18n.language?.startsWith('kn') ? 'ಡೆವಲಪ್‌ಡ್ ವಿತ್ ❤️ ಬೈ' : 'Developed with ❤️ by'}{' '}
                    <Link target="_blank" rel="noopener noreferrer" sx={{ color: isDark ? BRAND.yellow2 : 'rgba(17,24,39,0.9)', fontWeight: 600, textDecoration: 'none' }}>
                        Digitalentio
                    </Link>
                </Typography>
            </Box>
